...
{
    public partial class FrmPrincipalControladores : Form
    {
        FrmDiagramaDeBloques frmDiagramaBloques;
        Historial h;
        DataTable dat;

        //Estas variables van a controlar que la graficación lenta se haga una única vez
        bool[] graficaLentoMagnitud, graficaLentoFase;
        bool bandera;
        ...
        /// <summary>
        /// Lista de puntos de ancho de banda de ZedGraph.
        /// </summary>
        private List<LineItem> lineItemsPuntosAnchoBanda = new List<LineItem>();
        ...
        {
            if (chbPasosAutom.Checked == true)
            {
                this.retrocederEnPasoAutomático();
            }
            else
            {
                this.metodoBtnAtras_Click();
            }
        }
        ...
        //metodos para pasos automaticos
        private void WaitForMilliseconds(int ms)
        {
            Application.DoEvents();
            Thread.Sleep(ms);
        }
        private void avanzarEnPasoAutomático()
        {
            this.metodoBtnSig_Click();
            for (int i = 0; i < controladorBode.CurvasParciales.Count; i++)
            {
        ...
        private void metodoBtnSig_Click()
        {
            if (this.primeraEjecucion)
            {
                ...
                if (this.controladorBode.CurvasIndividuales.Count > 0 && this.controladorBode.CurvasParciales.Count > 0)
                {
                    ...
                    this.lineItemsPuntosAnchoBanda = generarLineItemsAnchoBanda();
                }
                ...
                graficaLentoFase = new bool[this.controladorBode.CurvasParciales.Count];
                graficaLentoMagnitud = new bool[this.controladorBode.CurvasParciales.Count];

                for (int i = 0; i < this.controladorBode.CurvasParciales.Count; i++)
                {
                    graficaLentoFase[i] = true;
                    graficaLentoMagnitud[i] = true;
                }
        ...
        private void metodoBtnAtras_Click()
        {
            if (this.controladorBode.CurvasIndividuales.Count > 0 && this.controladorBode.CurvasParciales.Count > 0)
                retroceder();
        }

        private void retrocederEnPasoAutomático()
        {
            ...
            this.chbPasosAutom.Enabled = true;
        }
        ...
        //{
        //    frmDiagramaBloques.K = null;
        //    frmDiagramaBloques.T1 = null;
        //    frmDiagramaBloques.T2 = null;
        //    frmDiagramaBloques.T3 = null;
        //    frmDiagramaBloques.T4 = null;
        //    frmDiagramaBloques.N1 = null;
        //    frmDiagramaBloques.N2 = null;
        //    frmDiagramaBloques.Td = null;
        //    frmDiagramaBloques.Wn = null;
        //    frmDiagramaBloques.Psi = null;
        //}
        ...
        public FrmPrincipalControladores()
        {
        ...
        //Hacer método generarLineItemsPuntosAnchoBanda. Poner visible false.
        /// <summary>
        /// Genera una lista de línea de ZedGraph, para el ancho de banda.
        /// </summary>
        /// <returns>Lista de líneas de ZedGraph, para el ancho de banda.</returns>
        private List<LineItem> generarLineItemsAnchoBanda()
        {
            List<LineItem> lineItems = new List<LineItem>();

            //Creamos dos LineItem, el primero es el punto del ancho de banda,
            //el segundo esta formado por tres puntos que marcan las coordenadas del ancho de banda.
            //Los agregamos al gráfico y enseguida los ocultamos.
            
            PointPairList pointPairList = new PointPairList();
            pointPairList.Add(this.controladorBode.AnchoBanda[0], this.controladorBode.AnchoBanda[1]);
            LineItem lineItem = this.gpMagnitud.AddCurve("Ancho de banda: " + this.controladorBode.AnchoBanda[0], pointPairList,
                Color.Black, SymbolType.Diamond);
            lineItem.IsVisible = false;
            lineItem.Label.IsVisible = false;
            ...
            pointPairList.Add(this.controladorBode.InicioEjeX, this.controladorBode.AnchoBanda[1]);
            pointPairList.Add(this.controladorBode.AnchoBanda[0], this.controladorBode.AnchoBanda[1]);
            pointPairList.Add(this.controladorBode.AnchoBanda[0], this.controladorBode.InicioEjeYMagnitud);
            LineItem lineItem2 = this.gpMagnitud.AddCurve("Ancho de banda: " + this.controladorBode.AnchoBanda[0], pointPairList,
                Color.Black, SymbolType.None);
            lineItem2.IsVisible = false;
            lineItem2.Label.IsVisible = false;

            lineItem2.Line.Style = System.Drawing.Drawing2D.DashStyle.Custom;
            lineItem2.Line.DashOn = 20;
            lineItem2.Line.DashOff = 5;
        ...
        private void mostrarCurvas()
        {
            ...
            //Definimos un tamaño de tramo por si está habilitada la opción de Graficar Lentamente
            int tamañoTramo = 500;

            //Curvas parciales. Las curvas parciales son las negras que se van presentando a medida que se avanza en Bode
            ...
            for (int i = 0; i < cantidadCurvasParciales; i++)
            {
                if (i == this.indiceCurvaActual)
                {
                    ...
                    //Grafica lentamente las curva parciales y final de magnitud
                    if (this.chkGraficarLentamente.Checked && graficaLentoMagnitud[i])
                        {
                            btnFormula.Enabled = false;
                            btnLimpiar.Enabled = false;
                            btnAvanzar.Enabled = false;
                            btnRetroceder.Enabled = false;
                            chkGraficarLentamente.Enabled = false;
                            chbPasosAutom.Enabled = false;
                            btnGanancia.Enabled = false;
                            nudGanancia.Enabled = false;

                            this.reestablecerEscala();

                            List<PointPairList> curvasAuxiliares = new List<PointPairList>();

                            PointPairList linea = new PointPairList();

                            curvasAuxiliares.Add(linea);

                            LineItem li = this.gpMagnitud.AddCurve("", curvasAuxiliares[0], this.colorCurvaParcial, SymbolType.None);
                            li.Label.IsVisible = false;
                            li.Line.IsAntiAlias = true;
                            li.Line.IsSmooth = true;
                            li.Line.SmoothTension = 0.05F;
                            li.Line.Width = GROSOR_CURVA_PARCIAL;

                            PointPairList tramo = new PointPairList();

                            int u = 1;

                            for (int h = 0; h < this.lineItemsCurvasParcialesMagnitud[i].Points.Count; h++)
                            {


                                linea.Add(this.lineItemsCurvasParcialesMagnitud[i].Points[h]);

                                if (u*tamañoTramo == h)
                                {
                                        zgcMagnitud.Invalidate();
                            ...
                            graficaLentoMagnitud[i] = false;
                        }
                        //Fin grafica lentamente curva final de magnitud

                    this.lineItemsCurvasParcialesMagnitud[i].IsVisible = true;
                    this.lineItemsCurvasParcialesMagnitud[i].Label.IsVisible = true;

                    //Grafica lentamente las curvas parciales y final de fase
                    if (this.chkGraficarLentamente.Checked && graficaLentoFase[i])
                    {
                        btnFormula.Enabled = false;
                        btnLimpiar.Enabled = false;
                        btnAvanzar.Enabled = false;
                        ...
                        LineItem li = this.gpFase.AddCurve("", curvasAuxiliares[0], this.colorCurvaParcial, SymbolType.None);
                        li.Label.IsVisible = false;
                        li.Line.IsAntiAlias = true;
                        li.Line.IsSmooth = true;
                        li.Line.SmoothTension = 0.05F;
                        ...
                        for (int h = 0; h < this.lineItemsCurvasParcialesFase[i].Points.Count; h++)
                        {


                            linea.Add(this.lineItemsCurvasParcialesFase[i].Points[h]);
                        ...
                        graficaLentoFase[i] = false;
                    }
                    //Fin grafica lento curva de fase

                    btnFormula.Enabled = true;
                    btnLimpiar.Enabled = true;
                    btnAvanzar.Enabled = true;
                    btnRetroceder.Enabled = true;
                    chkGraficarLentamente.Enabled = true;
                    chbPasosAutom.Enabled = true;
            ...
            if ((cantidadCurvasParciales - 1) == indiceCurvaActual)
            {
                ...
                if (lineItemsPuntosAnchoBanda[0] != null)
                {
                    this.lineItemsPuntosAnchoBanda[0].IsVisible = true;
                    this.lineItemsPuntosAnchoBanda[0].Label.IsVisible = true;
                }

                if (lineItemsPuntosAnchoBanda[1] != null)
                {
                    this.lineItemsPuntosAnchoBanda[1].IsVisible = true;
                }
            ...
            else
            {
                ...
                if (lineItemsPuntosAnchoBanda[0] != null)
                {
                    this.lineItemsPuntosAnchoBanda[0].IsVisible = false;
                    this.lineItemsPuntosAnchoBanda[0].Label.IsVisible = false;
                }

                if (lineItemsPuntosAnchoBanda[1] != null)
                {
                    this.lineItemsPuntosAnchoBanda[1].IsVisible = false;
                }
        ...
        {
            ZedGraphControl zedC = new ZedGraphControl();

            zedC.GraphPane.CurveList = zgcFase.GraphPane.CurveList.Clone();
            
            h.addGrafica(this.Formula, dat.Copy());
        ...
        private void zgcMagnitud_Load(object sender, EventArgs e)
        {
