...
{
    public partial class FormRootLocusControladores : Form
    {
        ...
        FrmDiagramaDeBloques frmPrimerOrdenProporcional;
        ...
        public FormRootLocusControladores()
        {
        ...
        private void manejadorBotones()
        {
            if (pasoActual == 1)
            {
                this.cBoxAutomatico.Enabled = true;
            }
            else
            {
                this.cBoxAutomatico.Enabled = false;
            }
        ...
        private void pantalla5()
        {
            ...
            else
            {
                ...
                if (cbLineasAmortCte.Checked == true)
                    graficarRecta5Lentamente();
                else
                    graficarRectas5();
        ...
        private void graficarRecta5Lentamente()
        {
            List<PointPairList> curvas = rl.rectasLocus();
            ...
            //RollingPointPairList curvasAuxiliares = new RollingPointPairList(curvas[0].Count);

            if (curvas.Count != 0)
            {
                List<PointPairList> curvasAuxiliares = new List<PointPairList>();

                PointPairList ramaIzq = new PointPairList();
                PointPairList ramaDer = new PointPairList();

                curvasAuxiliares.Add(ramaIzq);
                curvasAuxiliares.Add(ramaDer);

                LineItem li = zedC.GraphPane.AddCurve("RL" + 0, curvasAuxiliares[0], Color.Red);
                li.Label.IsVisible = false;
                li.Symbol.Type = SymbolType.None;
                li.Line.IsAntiAlias = true;
                li.Line.IsSmooth = true;
                ...
                LineItem li2 = zedC.GraphPane.AddCurve("RL" + 1, curvasAuxiliares[1], Color.Red);
                li2.Label.IsVisible = false;
                li2.Symbol.Type = SymbolType.None;
                li2.Line.IsAntiAlias = true;
                li2.Line.IsSmooth = true;
                li2.Line.SmoothTension = 0.05F;
                li2.Line.Width = 1.70f;

                for (int j = 0; j < curvas[0].Count; j++)
                {
                    ramaIzq.Add(curvas[0][j]);
                    ramaDer.Add(curvas[1][j]);
                    zedC.AxisChange();
                    zedC.Invalidate();
        ...
        {
            if (cBoxAutomatico.Checked == true)
            {
        ...
        //    FrmDatosPrimerOrdenProporcionalIntegralDerivativo dat = new FrmDatosPrimerOrdenProporcionalIntegralDerivativo(polos,ceros,rl);
        //    dat.ShowDialog();
        ...
        private void ingresoDatos()
        {
            ...
            //if (frmPrimerOrdenProporcional ==null)
            //{
            //    this.bandera = false;
                //frmPrimerOrdenProporcional = 
                //frmPrimerOrdenProporcional.ShowDialog();
            //}
            //else
            //    frmPrimerOrdenProporcional.Show();
        ...
        private string zedC_PointValueEvent(ZedGraphControl sender, GraphPane pane, CurveItem curve, int iPt)
        {
            ...
            double hipotenusa;
            double valorTita;
            double coeficienteAmortiguado;
            double constanteTiempo;

            hipotenusa = Math.Sqrt(Math.Pow(pto.X, 2) + Math.Pow(pto.Y, 2));
            valorTita = Math.Acos((pto.X / hipotenusa));
            coeficienteAmortiguado = Math.Cos(Math.PI - valorTita);
            constanteTiempo = 1 / hipotenusa;
            ...
            {
                respuesta = "X: " + Math.Round(pto.X, 3) + "\r\nY: " + Math.Round(pto.Y, 3) + "\r\nValor K: " + pto.Z + "\r\nCoef. Amort.: " + Math.Round(coeficienteAmortiguado, 3);

                if ((clicGrafica))
                {
                    if (coeficienteAmortiguado > 0)
                    {
                        DialogResult result = MessageBox.Show("¿Desea graficar la Respuesta Transitoria Escalón de Segundo Orden con Coef. Amort. de " + Math.Round(coeficienteAmortiguado, 3) + " ?", "Graficar Respuesta Transitoria", MessageBoxButtons.YesNo, MessageBoxIcon.Question);

                        if (result == DialogResult.Yes)
                        {
                            frmRespuestaTransienteLocus respTrans = new frmRespuestaTransienteLocus("Escalon2");
                            respTrans.coefAmort = coeficienteAmortiguado;
                            respTrans.constTiempo = constanteTiempo;
                            respTrans.agregarDatos();
                            respTrans.ShowDialog();
                        }
                    }
                    else if (coeficienteAmortiguado < 0)
                    {
                        MessageBox.Show("No se permite graficar Respuesta Transitoria con Coef. Amort. < 0 ", "Coeficiente de Amortiguamiento Negativo", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                }
        ...
        private bool zedC_MouseDownEvent(ZedGraphControl sender, MouseEventArgs e)
        {
            clicGrafica = true;
            return clicGrafica;
        }

        private bool zedC_MouseUpEvent(ZedGraphControl sender, MouseEventArgs e)
        {
            clicGrafica = false;
            return clicGrafica;
        }
