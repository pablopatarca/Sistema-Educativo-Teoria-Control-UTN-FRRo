...
{
    public class ProporcionalIntegralDerivativo : IPropiedadesGrafica
    {
        ...
        public string Titulo
        {
            get
            {
                return "Controlador Proporcional Integral Derivativo";
            }
            set
            {
        ...
        public string[] NombreParametros
        {
            get
            {
                string[] parametros = {"Error", "Amplitud", "Ganancia", "T. Derivativo", "T. Integral", "ps" };
        ...
        public List<PointPairList> generarPuntos(double[] parametros)
        {
            ...
            int error;
            double amplitud, kc, tau_d, tau_i, ps, x, y=0;
            ...
            switch (error)
            {
                ...
                case 2: for (double i = 0; i <= 40; i += 0.1)
                        {
                            x = i;
                            y = kc * amplitud * i + kc * tau_d * amplitud + (kc / tau_i) * 0.5 * amplitud * Math.Pow(i, 2) + ps;
                            datos.Add(x, y);
                        }
                        l_fin_eje_x = 10;
                        l_fin_eje_y = kc * tau_d * amplitud + ps + 50;
                        break;

                case 3: for (double i = 0; i <= 40; i += 0.1)
                        {
                            x = i;
                            y = kc * amplitud * Math.Pow(i, 2) + kc * tau_d * 2 * amplitud * i + (kc / tau_i) * (1 / 3) * amplitud * Math.Pow(i, 3) + ps;
                            datos.Add(x, y);
                        }
                        l_fin_eje_x = 8;
                        l_fin_eje_y = ps + 45;
